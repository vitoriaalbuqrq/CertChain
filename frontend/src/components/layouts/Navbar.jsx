...
const Navbar = () => {
  ...
  return (
    <nav className="bg-dark-background text-white font-bold flex justify-between items-center h-14 md:h-20 px-10 md:px-20">
      ...
      <ul className="gap-10 list-none hidden md:flex">
        ...
        <li><Link to="/registerInstitution" className="bg-primary font-semibold text-dark-background px-3 py-2 rounded-full hover:bg-primary-hover transition-all">Registrar Instituição</Link></li>
      </ul>
      <div onClick={handleNav} className='block md:hidden'>
          {nav ? <AiOutlineClose size={20}/> : <AiOutlineMenu size={20} />}
      </div>
      <div className={nav ? 'fixed left-0 top-0 p-4 w-[70%] h-full z-[10] bg-dark-background ease-in-out duration-500' : 'ease-in-out duration-500 fixed left-[-100%]'}>
        <Link to="/" className="text-primary text-xl font-bold m-4">CertChain</Link>
        <ul className="p-4 flex flex-col gap-6 list-none">
          <li><Link to="/validate" className="hover:opacity-90 hover:text-primary transition">Validar</Link></li>
          <li><Link to="/issue" className="hover:opacity-90 hover:text-primary transition">Emitir</Link></li>
          <li><Link to="/registerInstitution" className="bg-primary font-semibold text-dark-background px-3 py-2 rounded-full hover:bg-primary-hover transition-all flex flex-nowrap justify-center">Registrar Instituição</Link></li>
        </ul>
      </div>
